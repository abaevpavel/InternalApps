...
## 🔖 Хэндофф (сессия 2026-07-02)

**n8n-планировщик:**
- Дев-пайплайн перенесён в **прод-аккаунт basement** (импорт воркфлоу `cit7Gah53xPLLbdy`).
  Ключевое отличие дев→прод: убраны `Structured Output Parser` (падали `Model output
  doesn't fit format`), заменены на толерантные code-ноды `Parse Skills/Route/Insert`;
  все 3 агента на **gpt-5**. Матчер со скилл-уровнями уже был в проде.
- ✅ Проверено вживую: прогон `189989` — 16 задач (13 назначены + 3 через инсертер),
  **0 unscheduled**, запись в `AI_teams_schedule` прошла. Мозг на basement работает.
- Осталось по n8n: Slack-доставка (включить воркфлоу `aa6XaAQ6` + `TEAM_TO_USER`).

**Фронт:**
- Убрана встроенная дубль-страница «My Applications» (наследие до-портальных времён);
  корень `/` и `*` → `/tasks`. Task Planner теперь чисто планировщик.
- **Google Maps travel починен**: добавлен `localhost:5173` в referrer-ключа → travel
  считается реально (с трафиком). Осталось создать таблицу `travel_cache` в TP-Supabase
  (`dhtewaqfcsejdllwhgtl`) — SQL готов, убирает 404 и включает кэш.
- **Клиентские правки:** овертайм показывается с величиной (`overtime +Xh Ym`) на Proposed
  и Scheduled; дата расписания на Proposed-вью; Create Task — дата по умолчанию =
  следующий рабочий день (пропуская выходные).
- **Admin → Team:** починено отображение ролей (был баг ключа `p.id` vs `user_id`);
  убрана кнопка «Sync Team Accounts»; добавлена **«Set password»** — задать/сбросить
  пароль ПМ вручную + назначить роль. Через Edge Function `set-team-password`
  (super_admin only). Вход по email+паролю теперь полностью рабочий.
  - ⚠️ Особенности auth этого проекта (новые ключи Supabase) — в `docs`/памяти
    (`task-planner-edge-fn-auth`): токен шлём в теле, проверяем через `/rest`.
...
### ✅ n8n-планировщик — ГОТОВ (мозг собран и работает)
> Уточнено 2026-07-02: планировщик стабилен на инстансе **basement** (тег­гер/матчер/
> уровни скиллов/round-trip уже решены). Прежние июньские пункты про падающий тег­гер и
> переезд на t3d — **неактуальны**. Единственная оставшаяся n8n-задача — миграция аккаунта.

- [ ] **Перенести воркфлоу с аккаунта `t3d-projects` → аккаунт `basement`** (n8n) и указать
      фронту прод-вебхуки basement: `VITE_N8N_PLANNER_WEBHOOK`, `VITE_N8N_SLACK_WEBHOOK`.
- [ ] **Slack-доставка**: включить воркфлоу (Disabled) + заполнить `TEAM_TO_USER` реальными
      Slack ID (тестировать после переноса аккаунта).
