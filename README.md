...
docs/
  ...
  TASK-PLANNER-ROLES.md  Роли внутри Task Planner (админ ↔ бригадир): модель + план UI-работ.
...
5. **Единая оболочка.** Хедер, бургер-меню и экран App Settings — общие, из портала.
   Меню разделено на два блока: **блок текущей апки** (её экраны + `App Settings`, т.е.
   настройки именно этой апки) и **`PORTAL`** (`My Account`, `Portal Settings`), затем
   `Sign out`. На главной портала блока апки нет. Лого в хедере всегда ведёт на главную
   (отдельный пункт `My Applications` не нужен). Структура — `docs/PORTAL.md` §1.
6. **Настройки апки — в БД, не в коде.** Вебхуки и параметры лежат в `app_settings`
   ...
   env — только фолбэк.
7. **Весь интерфейс — на английском.** Любой текст, который видит пользователь: подписи,
   кнопки, вкладки, подсказки, плейсхолдеры, пустые состояния, **сообщения об ошибках**
   (в т.ч. из `services/*` и `throw new Error(...)`), а также названия ролей и пунктов меню
   в `appRegistry.ts` и `detail`-строки в Resources. Русский остаётся только в комментариях
   к коду, коммитах и `docs/` — там он рабочий язык команды. Новый экран не считается
   готовым, если в нём есть русская строка на UI.
