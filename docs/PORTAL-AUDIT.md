...
### ⚪ SEC-3. Свободные RLS — РАЗОБРАНО, ПРИНЯТЫЙ РИСК (2026-07-23)
Разведка `pg_policies` (55 политик) показала: **почти всё уже скоупнуто корректно** — checklists*,
employee* (сотрудники/прогресс/назначения/типы), projects, production_*, project_* закрыты
`user_has_admin_role` + `user_has_application_access` (у `employees` даже явный «Block anonymous»).
Единственная таблица с «любой authenticated» — **`email_templates`** (Sales): 4 политики `{public}`
с `auth.role()='authenticated'`. Решено **оставить как есть**: внутренний инструмент, доверенный круг,
контент несекретный (болванки offer-писем, не PII). Готовый фикс (2 политики admin+app-access по
`/sales-email-sender`) описан в истории — применить, только если в портал пустят внешних/подрядчиков.
...
### ⚪ SEC-5. Публичные бакеты — ПРИНЯТЫЙ РИСК (2026-07-23)
`production-checklist-photos`, `checklist-item-photos`, `checklist-photos`, `buildertrend-schedule-photos` —
public: фото по прямой ссылке без авторизации. Решено **оставить**: внутренний инструмент, ссылки наружу
не расходятся. Фикс на будущее (если понадобится): private + signed URLs.
...
### ✅ BUG-1. Прогресс не атомарен — ИСПРАВЛЕНО и ПРИМЕНЕНО (2026-07-23)
...
UNIQUE-индексы) прогнана на боевой БД HR DASHBOARD (Success, no rows returned); оба `upsertProgress`
переведены на `upsert(onConflict)`. Код и БД синхронны.
...
### ✅ BUG-3. Назначения без UNIQUE — ИСПРАВЛЕНО и ПРИМЕНЕНО (2026-07-23)
Было: `project_checklists` / `employee_checklists` без UNIQUE `(…_id, checklist_id)`, защита от дублей
проверкой в коде (гонка). Сделано: UNIQUE-индексы в миграции 0005 (применена); `assignChecklist` (hr) →
идемпотентный `upsert(onConflict, ignoreDuplicates)`. `assignTemplate` (project) остаётся delete+insert
(см. хвост миграции).
...
3. ✅ **UNIQUE-констрейнты** (BUG-1/BUG-3) — СДЕЛАНО, миграция 0005 применена на боевой БД (2026-07-23).
4. ⚪ **RLS-сужение** (SEC-3) — РАЗОБРАНО: всё уже скоупнуто, кроме `email_templates`; её оставили открытой осознанно (внутр. инструмент).
5. ⚪ **Приватные бакеты** (SEC-5) + `gmail-auth` (SEC-4) — оба принятый риск (внутр. инструмент).
