...
| **BUG-8** | **Профиль без `user_id` — тупик.** `getMyProfile` находит строку и по email, поэтому такой человек проходит whitelist-гейт, но `useAppAccess` получает `userId = null` → пустой список апок и «Access denied» на всех роутах. Из UI не чинится: селектор роли в Edit User для такой строки disabled. Сюда же дубль «Pavel Abaev» (§8): выборка идёт с `.limit(1)` без сортировки, победитель недетерминирован | ✅ **Исправлено** (2026-09-03, выкачено): три правки, чтобы тупик не воспроизводился ни при каком раскладе — `getMyProfile` сортирует `user_id` nulls last (при дубле выигрывает слинкованная строка); при входе непривязанная строка линкуется к аккаунту (`linkProfileToAuthUser`, `.select()` ловит тихий no-op от RLS); доступ считается по новому `effectiveUserId` (профиль либо id аккаунта) — одинаково в `useAppAccess` и `MyApplications`, поэтому тупик снят даже если линковку зарежет RLS |
| **BUG-9** | **Sales: картинка в письме роняет экран.** В тулбаре Quill включена кнопка `image` → base64 попадает в HTML и на каждый keystroke пишется в localStorage (квота ~5 МБ). При переполнении `setItem` бросает прямо из `useEffect`, обработчика нет. Та же строка целиком уезжает в вебхук | ✅ **Исправлено** (2026-09-03, выкачено): запись черновика обёрнута в `try/catch` (черновик — удобство, не данные), кнопка `image` убрана из тулбара Quill — существующие шаблоны с картинками по-прежнему открываются. Заодно после Send показывается подтверждение со временем |
| **BUG-10** | **AI-импорт в Production Checklist не работает.** Общая edge читает `imageBase64` + `checklistId` и вставляет пункты сама, а Production шлёт `{image}` и ждёт `{items}` обратно, чтобы вставить на клиенте. HR-контракт правильный, Production — нет | ✅ **Исправлено** (2026-09-03, выкачено): вызов переписан под настоящий контракт функции (`imageBase64` + `checklistId` + `table`), вставку делает сама edge — как в HR; ручной цикл вставки на клиенте убран. Кнопка заработала впервые, нужен живой прогон |
...
**Закрыто в этом заходе:** SEC-9 (три функции задеплоены), SEC-10 (миграция `0009` применена
на боевой), BUG-2, BUG-7, BUG-8, BUG-9, BUG-10 — всё выкачено на прод. Незакрытых находок
этого захода не осталось; из старых долгов открытыми остаются BUG-4, BUG-5, BUG-6 и SEC-6.
